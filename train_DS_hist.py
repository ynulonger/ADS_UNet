...
def evaluate(criterion, net, loader, device, classes, loss_fn,mask_flag):
    ...
    for batch in loader:
        with torch.no_grad():
            ...
            mask_list = [F.avg_pool2d(true_masks, kernel_size=2, stride=2), F.avg_pool2d(true_masks, kernel_size=4, stride=4),
                         F.avg_pool2d(true_masks, kernel_size=8, stride=8), F.avg_pool2d(true_masks, kernel_size=16, stride=16),
                         F.avg_pool2d(true_masks, kernel_size=8, stride=8), F.avg_pool2d(true_masks, kernel_size=4, stride=4),
                         F.avg_pool2d(true_masks, kernel_size=2, stride=2), true_masks]
            pred_list = net(imgs)
...
def train_net(net, device, train_dataset, val_dataset, epochs, batch_size, 
    ...
    for epoch in range(epochs):
        ...
        if val_miou.mean()>best_miou:
            ...
            torch.save(net.state_dict(), dir_checkpoint + f'{seed}_{lr}_DS_{loss_fn}_{mask_flag}.pth')
    ...
    writer = pd.ExcelWriter(f'{dir_checkpoint}/{seed}_{lr}_DS_{loss_fn}_{mask_flag}.xlsx')      
...
if __name__ == '__main__':
    ...
    net = DS_UNet(n_channels=3, n_classes=classes, filters=64, bilinear=False).cuda()
