...
from unet.Adaboost_UNet import AdaBoost_UNet_nn as AdaBoost_UNet
# from unet.Adaboost_UNet import AdaBoost_UNet_d_BCSS as AdaBoost_UNet
...
def evaluate(criterion, net, loader, device, classes, level, loss_fn, img_size):
    net.eval()
    iou = 0
    loss = 0
    length = 0
    ...
    for batch in loader:
        with torch.no_grad():
            ...
            for i in range(0,len(etas)):
                preds += F.interpolate(F.softmax(pred_list[i],dim=1), size=img_size, mode='bilinear', align_corners=True)*etas[i]
...
def compute_loss(criterion, pred_list, mask_list, net, weight, device, level, loss_fn):
    ...
    if level==1:
        ...
        alpha_temp = (F.softmax(net.alpha_5.view(-1), dim=0)+epslion)
        alpha_vector = alpha_temp/alpha_temp.sum()
    elif level==6:
        alpha_temp = (F.softmax(net.alpha_6.view(-1), dim=0)+epslion)
        alpha_vector = alpha_temp/alpha_temp.sum()
    elif level==7:
        alpha_temp = (F.softmax(net.alpha_7.view(-1), dim=0)+epslion)
        alpha_vector = alpha_temp/alpha_temp.sum()
...
def train_net(net, device, train_dataset, val_dataset, epochs, batch_size, lr,level, 
            skip_option, loss_fn, dir_checkpoint, deep_sup, img_size, classes, d_name, seed):
    ...
    for epoch in range(epochs):
        if level==1:
            ...
            alpha_temp = (F.softmax(net.alpha_5.view(-1), dim=0).cpu().detach().numpy()+epslion)
            alpha_vector = alpha_temp/alpha_temp.sum()
        elif level==6:
            alpha_temp = (F.softmax(net.alpha_6.view(-1), dim=0).cpu().detach().numpy()+epslion)
            alpha_vector = alpha_temp/alpha_temp.sum()
        elif level==7:
            alpha_temp = (F.softmax(net.alpha_7.view(-1), dim=0).cpu().detach().numpy()+epslion)
            alpha_vector = alpha_temp/alpha_temp.sum()
        ...
        train_miou, train_loss, train_hidden_loss = evaluate(criterion, net, train_loader, device, classes, level, loss_fn, img_size)
        if d_name=='Kumar':
            val_miou, val_loss, val_hidden_loss = evaluate(criterion, net, val_loader, device, classes, level, loss_fn, (992,992))
        else:
            val_miou, val_loss, val_hidden_loss  = evaluate(criterion, net, val_loader, device, classes, level, loss_fn, img_size)
        ...
        if val_miou.mean()>best_miou:
            ...
            torch.save(net.state_dict(), f'{dir_checkpoint}{seed}_{loss_fn}_{lr}_Ada_{skip_option}.pth')
    ...
    writer = pd.ExcelWriter(f'{dir_checkpoint}{seed}_{loss_fn}_{lr}_Ada_{skip_option}_{level}.xlsx')
...
def get_args():
    ...
    parser.add_argument('-d', '--data', dest='data', type=str, default='BCSS',
                        help='dataset')
    parser.add_argument('-m', '--model', dest='model', type=str, default='Ada',
                        help='MODEL NAME')
    parser.add_argument('-s', '--seed', dest='seed', type=str, default=34, 
                        help='mask')
    parser.add_argument('-ds', '--supervision', dest='supervision', type=bool, default=True, 
                        help='mask')
...
def eval_UNet(net, val_dataset, device, batch_size, class_num, img_size):
    net.eval()
...
if __name__ == '__main__':
    ...
    elif args.data == 'Kumar' or args.data=='GlaS':
        classes = 2
        img_size= [400, 400]
    ...
    dir_checkpoint = 'checkpoints/'+args.data+'/'+args.model+'_'+str(deep_sup)+'_'+args.skip+'_deep/'
    ...
    for t in range(1,8):
        print(f'===========================iter:{t}==============================')
        net = AdaBoost_UNet(n_channels=n_channels, n_classes=classes, level=str(t), skip_option=args.skip, filters=8, deep_sup=deep_sup).cuda()
        ...
        if t==1:
            ...
            if os.path.exists(f'{dir_checkpoint}{args.seed}_{args.loss}_{args.lr}_Ada_{args.skip}.pth'):
                print(f'loading weight from {dir_checkpoint}{args.seed}_{args.loss}_{args.lr}_Ada_{args.skip}.pth')
                net.load_state_dict(torch.load(f'{dir_checkpoint}{args.seed}_{args.loss}_{args.lr}_Ada_{args.skip}.pth', map_location=device))
            else:
        ...
        net, train_hidden_loss = train_net(net=net, device=device, train_dataset= train_dataset, val_dataset=val_dataset,
                ...
                deep_sup=deep_sup, img_size=img_size, classes=classes, d_name=args.data, seed=args.seed)
        
        print(f'loading weight from {dir_checkpoint}{args.seed}_{args.loss}_{args.lr}_Ada_{args.skip}.pth')
        net.load_state_dict(torch.load(f'{dir_checkpoint}{args.seed}_{args.loss}_{args.lr}_Ada_{args.skip}.pth', map_location=device))

        head = np.argmin(train_hidden_loss)
        start_e = time.time()
        mIoU_list, w_err, w_miou = eval_UNet(net, train_dataset, device, args.batch_size, classes, img_size)
        end_e = time.time()
    ...
    f = open(f'{dir_checkpoint}{args.seed}_{args.loss}_{args.lr}_Ada_{args.skip}.txt', 'w')
    ...
    writer = pd.ExcelWriter(f'{dir_checkpoint}{args.seed}_{args.loss}_{deep_sup}_{args.lr}_sample_weight.xlsx')
