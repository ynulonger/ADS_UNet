...
def train_net(net, device, train_dataset, val_dataset, epochs, batch_size, lr,level, 
            skip_option, loss_fn, dir_checkpoint, deep_sup, img_size, classes, d_name):
    ...
    for epoch in range(epochs):
        ...
        if val_miou.mean()>best_miou:
            ...
            torch.save(net.state_dict(),
                    f'{dir_checkpoint}{loss_fn}_{lr}_Ada_{skip_option}.pth')
...
def eval_UNet(net, val_dataset, device, batch_size, class_num):
    ...
    for batch in val_loader:
        with torch.no_grad():
            ...
            preds=0
            for i in range(0,len(etas)):
                # preds += F.interpolate(torch.sigmoid(pred_list[i]), size=(512,512), mode='bilinear', align_corners=True)*etas[i]
                preds += F.interpolate(F.softmax(pred_list[i],dim=1), size=img_size, mode='bilinear', align_corners=True)*etas[i]
          
            # preds = (preds>=0.5).squeeze(1)
            # masks = (masks>=0.5).squeeze()
...
if __name__ == '__main__':
    ...
    print(f'-------dataset: {args.data}, re-weight: {args.re_weight}, skip_option:{args.skip}, epochs: {args.epochs}, batch_size:{args.batch_size}')
    device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    alphas = []

    # dir_checkpoint = 'checkpoints/'+args.data+'/sample_weight/'
    dir_checkpoint = 'checkpoints/'+args.data+'/ds_'+str(seed)+'_'+str(deep_sup)+'_'+args.re_weight+'_'+args.skip+'/'
    ...
    for t in range(1,5):
        ...
        net, train_hidden_loss = train_net(net=net, device=device, train_dataset= train_dataset, val_dataset=val_dataset,
                ...
                deep_sup=deep_sup, img_size=img_size, classes=classes, d_name=args.data)
    ...
    writer = pd.ExcelWriter(f'{dir_checkpoint}{args.loss}_{deep_sup}_{args.lr}_sample_weight.xlsx')
