...
def train_net(net, device, train_dataset, val_dataset, epochs, batch_size, lr,level, 
    ...
    for epoch in range(epochs):
        ...
        if val_miou.mean()>best_miou:
            ...
            torch.save(net.state_dict(),
                    f'{dir_checkpoint}{loss_fn}_{lr}_Ada_{skip_option}_cons_unens.pth')
    ...
    writer = pd.ExcelWriter(f'{dir_checkpoint}{loss_fn}_{lr}_Ada_{skip_option}_{level}_cons_unens.xlsx')
...
if __name__ == '__main__':
    ...
    dir_checkpoint = 'checkpoints/'+args.data+'/cons_unens/'
    ...
    for t in range(1,5):
        ...
        if t==1:
            ...
            train_dataset, val_dataset = get_data(train_sample_weight, args.data)            
            if os.path.exists(f'{dir_checkpoint}{args.loss}_{args.lr}_Ada_{args.skip}_cons_unens.pth'):
                print(f'loading weight from {dir_checkpoint}{args.loss}_{args.lr}_Ada_{args.skip}_cons_unens.pth')
                net.load_state_dict(torch.load(f'{dir_checkpoint}{args.loss}_{args.lr}_Ada_{args.skip}_cons_unens.pth', map_location=device))
            else:
    ...
    f = open(f'{dir_checkpoint}{args.loss}_{args.lr}_Ada_{args.skip}_cons_unens.txt', 'w')
    for alpha in alphas:
        f.write(str(alpha))
        f.write("\n")
    ...
    writer = pd.ExcelWriter(f'{dir_checkpoint}{args.loss}_{deep_sup}_{args.lr}_cons_unens.xlsx')
