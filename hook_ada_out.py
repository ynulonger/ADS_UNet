...
# state_dict = torch.load(f'checkpoints/BCSS/sample_weight/ce_0.002_Ada_scse.pth', map_location=device)
state_dict = torch.load(f'checkpoints/BCSS/Ada_True_scse/34_ce_0.001_Ada_scse.pth', map_location=device)
...
classes = 5
nodes = 4
model =  AdaBoost_UNet(n_channels=3, n_classes=classes, filters=64, skip_option='scse', level='1234_feat').cuda()

model.to(device=device)
# model.load_state_dict(new_state_dict)
model.load_state_dict(state_dict)

# test_imgs = np.array(glob.glob(f'/ssd/BCSS-2021/test_imgs/*png'))
# test_masks = np.array(glob.glob(f'/ssd/BCSS-2021/test_gts/*png'))
# idx = [i for i in range(len(test_imgs))]
# print(idx)
# np.random.seed(2)
# np.random.shuffle(idx)
# test_imgs = test_imgs[idx][:2000]
# test_masks = test_masks[idx][:2000]
# val_dataset   = HistImageSet(test_imgs, test_masks, sample_weight=None, mask_channel=5)
...
get_similarity_matrix(svcca, f'ada_out')
# get_similarity_matrix(svcca, f'cascade_out({skip})')
