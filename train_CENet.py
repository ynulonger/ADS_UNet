...
def get_args():
    ...
    parser.add_argument('-b', '--batch_size', dest='batch_size', type=int, default=4,
                        help='batch size')
    parser.add_argument('-m', '--model', dest='model', type=str, default='CENet',
                        help='model')
...
if __name__ == '__main__':
    ...
    if args.data == 'BCSS':
        ...
        classes =2
        img_size= [512,512]
        train_dataset = CRAG('train')
        test_dataset  = CRAG('test')
    ...
    # flops, params = profile(net, inputs=(inputs, ), custom_ops={UNet2Plus: count_your_model})
    flops, params = profile(net, inputs=(inputs, ), custom_ops={CENet: count_your_model})
