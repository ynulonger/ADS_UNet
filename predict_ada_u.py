...
def predict_img(model, net, device, dataset, alphas, level, test_fold=None, vote=None):
    print('-------', level)
    ...
    print(f'fold_{test_fold}, branch:{level}, vote:{vote}, mean: {miou.mean()}')
    # print(miou)
    # miou_list = metrics._iou_list
    # cm = metrics._confusion_matrix
    # print(f'fold_{test_fold}, branch:{level}, vote:{vote}, mean: {miou_list.mean()}')
...
if __name__ == "__main__":
    ...
    for line in f:
        ...
        i+=1 
    if len(args.level) != 1:
        net = AdaBoost_UNet(n_channels=1, n_classes=28, level=args.level,
            filters=16, skip_option=args.skip, deep_sup=False).cuda()
    else:
        net = AdaBoost_UNet(n_channels=1, n_classes=28, level=str(args.level),
            filters=16, skip_option=args.skip, deep_sup=False).cuda()

    # args.level = heads
    print(args.skip, alphas, args.level, args.vote)

    fold = int(args.weight.split('_')[3])
    ...
    predict_img(args.model, net, device, 'kylberg', alphas, args.level, test_fold=fold, vote=args.vote)
