...
def predict_img(model, net, device, dataset, alphas, level, heads, classes, vote=None):
    ...
    print('samples:', test_dataset.__len__())
        ...
        with torch.no_grad():
            ...
            if not isinstance(level, list):
                ...
                for i in range(0,len(eta)):
                    ...
                    # preds += F.interpolate(F.softmax(output[i],dim=1), size=(512,512), mode='nearest')*eta[i]
                ...
                # soft voating
                    ...
                    for i in range(0, len(alphas)):
                        ...
                        for j in range(0,len(eta)):
                            branch_pred += F.interpolate(F.softmax(output[j],dim=1), size=(imgs.size(2),imgs.size(3)), mode='bilinear', align_corners=True)*eta[j]
            ...
            for i in range(preds.shape[0]):
                ...
                error_map = cv2.applyColorMap(error_map, cv2.COLORMAP_RAINBOW)
                cv2.imwrite(save_patches+'/'+names[i].split('.')[0]+'.png', preds[i])
                count+=1
                # cv2.imwrite(save_patches+'/m_'+names[i], preds[i])
                # print(temp_iou)

    print(f'branch:{level}, vote:{vote}, miou:{metrics.iou(average=True)}, dice: {metrics.dice(average=True)},{metrics.iou(average=False)}')
...
def get_args():
    ...
    parser.add_argument('--gpu', '-g', metavar='FILE', type=str, default='0',
                        help="Specify the file in which the model is stored")
    parser.add_argument('--filters', '-f', metavar='FILE', type=int, default=32,
                        help="Specify the file in which the model is stored")
